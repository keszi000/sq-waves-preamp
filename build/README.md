...
- **build-windows.sh** — Builds `sqapi.exe` with GUI subsystem (no console window). Use Git Bash or WSL on Windows. For an icon and version info in the exe, create `winres/icon.png` (e.g. 256×256 from `static/icon.svg`) and run `go-winres make` before `go build`; CI does this automatically.
